...
class Robot():
    ...
    def __init__(self):
        ...
        self.find_human_pub = rospy.Publisher("/find_human",Bool,queue_size = 10)
        self.follow_human_sub = rospy.Subscriber("/follow_human",String,self.followHumanCallback)
        #public
        #ローカル座標
        self.local = Pose()
        self.world = Pose()
    ...
    def welcomeMessage(self):
        rospy.loginfo("Follow me program by demura lab.,KIT")
        rospy.loginfo(kFollowMaxDistance)
        rospy.loginfo(kFollowMinDistance)
        rospy.loginfo(kFollowAngle)
    ...
    def calcHumanPoseVer2(self,object_num, object, human_obj):
        ...
        if object_num == 0 or object_num == 1:
            ...
            if (d < kLegBetweenDistance) and (human_pos_judge):
                ...
                self.find_human_pub.publish(False)
    ...
    def followHuman(self,input_image,moveble = True):
        ...
        self.find_human_pub.publish(True)
